...
public:
BottomUpBFSSelector(int* _q2, int* _q2c, int64_t* _pred_glob): q2(_q2), q2c(_q2c), pred_glob(_pred_glob) {
...
void run_bfs_selector(int64_t root, int64_t* pred) {
    ...
    while (sum) {
        BottomUpBFSSelector *bfss_ptr = new BottomUpBFSSelector(q2, &q2c, pred_glob);
        hclib::finish([=, &q2c]() {
                bfss_ptr->start();
